---
document_id: AEGIS-PREPROC-DORA-ART-9
title: DORA Art. 9 — SecurityObjectives & SecurityRules
regulation: DORA
article: Art. 9
applicable: true
version: 0.1
...
|---|---|---|---|
| SO-DORA-001 | Data confidentiality, integrity, authenticity and availability are maintained at high standards across all data states — at rest, in use, or in transit — through ICT security policies, procedures, protocols and tools designed to ensure ICT-system resilience, continuity and availability. | `DORA-C09` (Art. 9(2) CIA+A 4-way AND + 3-way OR of data states); `DORA-C10` (Art. 9(2) in-transit, same locus); `DORA-C11` (Art. 9(2) integrity, same locus) | D-01.1, D-01.2, D-01.4 |
| SO-DORA-001 | D-01.1, D-01.2, D-01.4 | Art. 9(2) | CIA + A maintained across data states (at rest, in use, in transit) |
| SO-DORA-002 | Strong authentication mechanisms (based on relevant standards and dedicated control systems), cryptographic-key protection measures, and data encryption based on approved data classification and ICT risk assessment processes are implemented through coordinated policies and protocols. | `DORA-C15` (Art. 9(4)(d) 3-control AND: strong auth + key protection + encryption); `DORA-C16` (Art. 9(4)(d) same locus — `strong authentication` dominant reading is MFA per EBA guidelines); `DORA-C17` (Art. 9(4)(d) same locus — key management sub-phrase) | D-01.3, D-03.1, D-03.2 |
| SO-DORA-002 | D-01.3, D-03.1, D-03.2 | Art. 9(4)(d) | Strong auth + crypto key protection + risk-based encryption |
| SO-DORA-004 | A sound network and infrastructure management structure using appropriate techniques, methods and protocols is established, with appropriate and comprehensive documented policies for patches and updates maintained. | `DORA-C13` (Art. 9(4)(b) — T4 maps to D-02.2 patch management but OJ text is network/infrastructure management; **T4 mapping mismatch** per `../Regulation/DORA/Ambiguity/05_DORA.md` §2.13 — SO follows OJ-literal locus); `DORA-C19` (Art. 9(4)(f) patches and updates) | D-02.2, D-07.1 |
| SO-DORA-004 | D-02.2, D-07.1 | Art. 9(4)(b) + Art. 9(4)(f) | Network/infrastructure mgmt structure + patch policies |
| SO-DORA-006 | ICT security policies limit physical or logical access to information assets and ICT assets to what is required for legitimate and approved functions and activities only. | `DORA-C14` (Art. 9(4)(c) physical/logical access + `legitimate and approved` AND + `only` qualifier) | D-03.3 |
| SO-DORA-006 | D-03.3 | Art. 9(4)(c) | Least-privilege access policies (physical or logical) |
| SO-DORA-013 | Documented policies, procedures and controls for ICT change management — covering changes to software, hardware, firmware components, systems or security parameters — ensure that all changes are recorded, tested, assessed, approved, implemented and verified in a controlled manner, based on a risk assessment approach. | `DORA-C18` (Art. 9(4)(e) 5-way OR of change-targets + 6-way AND of change-management steps: recorded/tested/assessed/approved/implemented/verified + `controlled manner`) | D-07.4, D-07.2 (partial) |
| SO-DORA-013 | D-07.4, D-07.2 (partial) | Art. 9(4)(e) | ICT change management policies, procedures and controls |
