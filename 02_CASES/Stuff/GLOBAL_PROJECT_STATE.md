...
**Version:** 8.7 (MASTER DASHBOARD richer tables R7 — 1-2 new columns per table in 9 DataTables (Type/Anchor/Threats/Activities/Type+Description/Threat+Type/Type/Likelihood+Impact) with `<th title="...">` tooltips on all headers; modal drill-down gains a mini-TOC (`.am-toc` with anchor links + scrollIntoView smooth) and a filter input (`.am-filter`) that hides sections + flat `<dt>/<dd>` pairs not matching the query (shows "No matches" when nothing matches); cross-dashboard links (`#folio{04..12}-crosslink`) with case-aware dynamic paths to `Case_0X_P{2,3}_Dashboard.html` and `UNIFIED/AEGIS_Dashboard.html` / `GDPR_Dashboard.html` / `CRA_Dashboard.html` / `QNRCS_Reg756_2026_Dashboard.html`; CSS `.am-toc / .am-filter / .am-folio-crosslink / .am-section[hidden] / .am-no-matches`; `fillFolioCrosslinks(c)` in `renderP1/P2/P3`; test_master_drilldown.py R7: 9 column checks + 1 modal TOC+filter (≥5 links, click scroll, "basic" filter hides non-matching) + 1 crosslink suite (9 anchors); regression 9/9 R5 + 3/3 R6 maintained; screenshot `master_drilldown_toc.png`; ANCILLARY TO R6 full-card drill-down: 9 tabelas click→modal com todos os campos; MASTER DASHBOARD widget fix R4: 17 paths/joins corrigidos; MASTER DASHBOARD FIX — radar + grafo 749 nós + ambiguity widgets P1↔P2↔P3; MASTER DASHBOARD — UNIFIED/AEGIS_Master_Dashboard.html (single-file 3-case × 3-phase + crosscase + 13 folios); scripts/build_master_dashboard_data.py → data/aegis_master_data.json (103 UCs / 98 PROC / 23 CAP / 187 rules); realization_class yaml (C1) + heuristic (C2/C3) anchored rubric v1.7 §8; smoke 17/17 + 10 Playwright vistas match counts; RENUMBER — flat 1..N lane ids, zero legacy: C3 UC-01..33 + PROC-01..50; C1 UC-01..40; C2 UC-01..36, rubric v1.10; MERMAID RENDER FIX — all diagrams render-validated: 26 UC diagrams → PlantUML+SVG hybrid, rubric v1.9, mermaid check 192/192; UC SEPARATION — lane-pure use-case catalogs across 3 cases, rubric v1.8: C3 Doc22 v3.0 33 UCs + PKG-DS, 15 re-lanes, PROC-39/40→UC-66/92; C1 Doc20 v3.2 23+17; C2 Doc21 v1.5 36; annex A UC-ovals-only 11/6/8, annex B 23/21/33; audit 100/100/100; 6 borderline → P7; UML diagrams — use case diagrams annex A + sequence annex B; Realization Class tag waves C2+C3 + objectives tagging + CAP maturity + F5 fixes + FR/NFR Phase B; ALT-ANCHOR — UNMAPPED_* retired across 3 cases; 5 frozen referential sources + 800-53r5 generator; 3 gates v0.4 PASS; REALIZATION-CLASS; PORT-PARITY-2 — cross-case content parity: C3 P1 v1.6 layer + corr-013, P2 waves C2+C3, P3 rich v0 C2+C3, 16/16 smoke, all gates PASS)
**Scope:** All Cases
...
**Render fix (2026-09-05, rubric v1.9, commit f3c62b9).** Real-render validation exposed
that `useCaseDiagram` never existed in Mermaid (#4628) — all 26 UC diagrams are now
**PlantUML + SVG hybrid** (source block + committed `annexes/svg/` rendered via the
PlantUML server); sequence diagrams repaired (`;` separators, `OFF` keyword collision).
Mermaid render check 192/192 OK; smoke 16/16; audit 100/100/100.

**RENUMBER (2026-09-05, rubric v1.10 §5B rule 7, commits bbe7a02/18f326c/3545c19/13945cf).**
All lane ids renumbered **flat 1..N per lane per case** — no gaps, no retired numbers,
dotted `U.C.x.y.z` flattened: C3 UC-01..33 + PROC-01..50 + CAP-01..10; C1 UC-01..40;
C2 UC-01..36. Registry: `00_METHODOLOGY/validation/RENUMBER_REGISTRY_2026-09-05.md`
(old→new, traceability record). Two-phase renames, zero content loss; SVGs re-rendered;
audit 100/100/100; residual old ids = provenance/history only.
