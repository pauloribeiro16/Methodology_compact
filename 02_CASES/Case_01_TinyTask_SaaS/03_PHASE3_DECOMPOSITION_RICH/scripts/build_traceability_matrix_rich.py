...
UC_FREEZE = [
    ...
    ("UC-07", "SEC", "D-04.2", "DoS Resilience"),
    ("PROC-05", "SEC", "D-04.3", "Incident Notification (24h)"),
    ("UC-08", "SEC", "D-04.4", "Data Restoration and Recovery"),
    ("UC-09", "IAM", "D-03.1", "User Authentication"),
    ("UC-10", "IAM", "D-03.2", "MFA for Privileged Accounts"),
    ("UC-11", "IAM", "D-03.3", "Authorisation / Least Privilege"),
    ("UC-12", "IAM", "D-03.4", "Secure System Defaults"),
    ("PROC-06", "IAM", "D-09.4", "Processing & Breach Records"),
    ("UC-13", "IAM", "D-10.2", "Audit Logging"),
    ("PROC-07", "IAM", "D-10.3", "Control Effectiveness Testing"),
    ("PROC-08", "DEV", "D-07.1", "Security by Design"),
    ("UC-14", "DEV", "D-07.2", "SAST/DAST in CI/CD"),
    ("UC-15", "DEV", "D-02.2", "Security Patch Deployment"),
    ("UC-16", "DEV", "D-04.1", "Fail-Safe Design"),
    ...
    ("UC-17", "GOV", "D-06.2", "SBOM Publication"),
...
FUNCTIONAL_UC_FREEZE = [
    ("UC-18", "ACC", "—", "Sign Up & Account Creation"),
    ("UC-19", "ACC", "—", "Login (email/password + optional SSO)"),
    ("UC-20", "ACC", "—", "Password Reset & Recovery"),
    ("UC-21", "ACC", "—", "Session Management (timeout, logout-everywhere)"),
    ("UC-22", "ACC", "—", "Invite Member & Assign Role"),
    ("UC-23", "CORE", "—", "Create Workspace"),
    ("UC-24", "CORE", "—", "Create Project"),
    ("UC-25", "CORE", "—", "Create Task"),
    ("UC-26", "CORE", "—", "Assign Task"),
    ("UC-27", "CORE", "—", "Change Task Status & Due Date"),
    ("UC-28", "CORE", "—", "View Project Board (Kanban)"),
    ("UC-29", "COLAB", "—", "Comment on Task"),
    ("UC-30", "COLAB", "—", "@Mention & In-App Notification"),
    ("UC-31", "COLAB", "—", "Attach File to Task"),
    ("UC-32", "COLAB", "—", "Search & Filter Tasks"),
    ("UC-33", "COLAB", "—", "Activity Feed (recent events)"),
    ("UC-34", "PLAT", "—", "Mobile Sync (offline-first)"),
    ("UC-35", "PLAT", "—", "Stripe Checkout (Upgrade Plan)"),
    ("UC-36", "PLAT", "—", "Workspace Admin Console"),
    ("UC-37", "PLAT", "—", "Enterprise SSO"),
    ("UC-38", "SELF", "—", "View My Account (data held)"),
    ("UC-39", "SELF", "—", "Export My Data (GDPR portability)"),
    ("UC-40", "SELF", "—", "Delete My Account / Workspace"),
]

# Sprint 6 NEW: 8 Misuse Cases (Sindre & Opdahl)
MUC_FREEZE = [
    ("MUC-01", "A-MIS-01", "UC-19, UC-20", "UC-09, UC-10, UC-06, UC-13", "Credential Stuffing Against Login"),
    ("MUC-02", "A-MIS-03", "UC-22, UC-36", "UC-11, UC-13, PROC-11", "Privilege Escalation via Invite/Roles"),
    ("MUC-03", "A-MIS-01/A-MIS-03", "UC-24, UC-25, UC-28, UC-23, UC-34", "UC-12, UC-11, PROC-03, UC-14", "Cross-Tenant Data Injection/Read"),
    ("MUC-04", "A-MIS-03", "UC-39, UC-32", "UC-02, UC-04, UC-07, UC-13", "Bulk Data Extraction via Export Endpoint"),
    ("MUC-05", "A-MIS-04", "UC-35, UC-19", "PROC-14, CAP-01, UC-09", "Compromised Third-Party Integration"),
    ("MUC-06", "A-MIS-02", "All UC-18..UC-40 (data plane)", "UC-10, UC-11, UC-13, UC-06", "Malicious Insider Exfiltration"),
    ("MUC-07", "A-MIS-01", "UC-28, All UC-18..UC-40 (availability)", "UC-07, UC-16, UC-08", "Board / Service DoS"),
    ("MUC-08", "A-MIS-01", "UC-31", "UC-06, UC-13, UC-14", "Malicious Attachment Upload"),
]

# Sprint 6 NEW: Functional UC → Security UC constraints (primary). Total = 35 (1 per security UC).
CONSTRAINS_EDGES = [
    ("PROC-01", "UC-38, UC-39, UC-40"),
    ("PROC-02", "UC-38"),
    ("UC-01", "UC-40"),
    ("UC-02", "UC-39"),
    ("UC-03", "UC-18, UC-38"),
    ("UC-04", "UC-39"),
    ("PROC-03", "UC-25, UC-31"),
    ("UC-05", "All UC-18..UC-40"),
    ("PROC-04", "All UC-18..UC-40"),
    ("UC-06", "UC-25, UC-31"),
    ("UC-07", "All UC-18..UC-40"),
    ("PROC-05", "All UC-18..UC-40"),
    ("UC-08", "All UC-18..UC-40"),
    ("UC-09", "UC-18, UC-19, UC-20, UC-21, UC-34"),
    ("UC-10", "UC-37, UC-36"),
    ("UC-11", "UC-22, UC-23, UC-24, UC-25, UC-26, UC-27, UC-28, UC-36"),
    ("UC-12", "All UC-18..UC-40"),
    ("PROC-06", "All UC-18..UC-40"),
    ("UC-13", "All UC-18..UC-40"),
    ("PROC-07", "All UC-18..UC-40"),
    ("PROC-08", "UC-23, UC-24, UC-25, UC-26, UC-27, UC-28, UC-34, UC-35, UC-36, UC-37"),
    ("UC-14", "UC-23, UC-24, UC-25, UC-26, UC-27, UC-28, UC-31"),
    ("UC-15", "All UC-18..UC-40"),
    ("UC-16", "UC-28, UC-32, UC-34"),
    ("PROC-09", "UC-35, UC-37, UC-38, UC-39, UC-40"),
    ("PROC-10", "All UC-18..UC-40"),
    ("PROC-11", "All UC-18..UC-40"),
    ("PROC-12", "UC-35, UC-37, UC-38, UC-39, UC-40"),
    ("PROC-13", "All UC-18..UC-40"),
    ("PROC-14", "UC-35, UC-19"),
    ("CAP-01", "UC-35, UC-19"),
    ("UC-17", "UC-23, UC-24, UC-25, UC-26, UC-27, UC-28, UC-34, UC-35, UC-36, UC-37"),
    ("PROC-15", "All UC-18..UC-40 (human-driven)"),
    ("PROC-16", "UC-22, UC-36"),
    ("PROC-17", "UC-18, UC-19"),
]
...
FR_TO_UC: Dict[str, List[str]] = {
    "FR-01": ["UC-11", "UC-13"], "FR-02": ["UC-09"],
    "FR-03": ["UC-12"], "FR-04": ["PROC-07"],
    "FR-05": ["UC-13"], "FR-06": ["UC-09", "PROC-06"],
    "FR-07": ["PROC-01"], "FR-08": ["UC-01"],
    "FR-09": ["UC-02"], "FR-10": ["UC-03"],
    "FR-11": ["UC-04"], "FR-12": ["UC-03"],
    "FR-13": ["PROC-03", "UC-08"], "FR-14": ["PROC-03"],
    "FR-15": ["UC-05"], "FR-16": ["U.C.1.6.1 (legacy)", "UC-05"],
    "FR-17": ["PROC-04"], "FR-18": ["UC-06"],
    "FR-19": ["PROC-05"], "FR-20": ["PROC-08", "UC-14", "UC-15"],
    "FR-21": ["UC-15"], "FR-22": ["UC-16"],
...
def build_funcuc_to_secuc(wb: Workbook) -> None:
    ...
    for i, (sec_uc, func_ucs) in enumerate(CONSTRAINS_EDGES, start=1):
        ctype = (
            ...
            else "Availability / release gating" if sec_uc.startswith(("UC-05","UC-06","UC-07","UC-08"))
            else "Authn/Authz" if sec_uc.startswith(("UC-09","UC-10","UC-11","UC-12","UC-13"))
            else "Secure development" if sec_uc.startswith(("UC-14","UC-15","UC-16"))
            else "Governance / DPA" if sec_uc == "UC-17"
            else "Awareness"
        )
