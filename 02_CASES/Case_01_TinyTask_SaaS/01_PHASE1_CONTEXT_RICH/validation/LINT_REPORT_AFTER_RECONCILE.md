...
title: Lint Report — Post-Reconciliation (Fase de Especificação 1, Rich Mode)
phase: 1
version: 1.0
created: 2026-08-06
author: Fase de Especificação 1 Executor (lint-runner)
status: RECONCILED
...
> **Snapshot of Phase 1 lint state AFTER Fase de Especificação 1 reconciliation of the Rich Mode folder.**
...
- **Target phase dir:** `01_PHASE1_CONTEXT_RICH/` (Rich Mode — 12 legacy Phase 1 docs + 2 NEW placeholders + 1 ontology, all copied/reconciled in Fase de Especificação 1)
...
- **Individual lint invocations:** `lint_company_context`, `lint_regulatory_mapping`, `lint_regulatory_references` have no `main()`/`argparse` CLI and are invoked by importing the `lint_*` function and calling it on the resolved case path (same as Fase de Especificação 0 baseline; the Fase de Especificação 0 wrapper pattern was reused).
...
1. **Fase de Especificação 1 reconciliation** moved 11 legacy Phase 1 docs into the Rich folder; some warnings (e.g., the 6 false-positive obligated-party tokens in `02_Regulatory_Mapping_Master.md`) disappear because that file is not copied to the Rich folder (it is a `00_COMMON/` artefact, not a Phase 1 doc).
...
- **Fase de Especificação 1 fix status:** **NOT FIXED** (both warnings are out of Fase de Especificação 1 scope per the task — Doc 07 coverage matrix and the 4 sole-authority gap rows are corpus/Fase de Especificação 2/3 concerns).
...
- **Fase de Especificação 1 fix status:** **PARTIALLY FIXED** (6 obligated-party false-positives eliminated because `02_Regulatory_Mapping_Master.md` is not in the Rich folder; the 1 remaining sole-authority mismatch is the D-02.3 table in `00_Taxonomy_Reference.md` which carries `GDPR-C04/C14, CRA-C07, DORA-C09` for D-01.1 and `CRA-C21/C26` for D-02.3 but is inconsistent with the ground truth in the case context; this is an ontology content question requiring human decision per P7, **out of Fase de Especificação 1 scope**).
...
- **Metrics:**
  ...
  - Domains covered: 3
  - Documents found: 1
- **Fase de Especificação 1 fix status:** **NOT FIXED** (the warning is now in `05_Regulatory_Applicability.md` (Rich) instead of `02_Regulatory_Mapping_Master.md` (legacy). The warning itself — "only 3 security domains mentioned" — is a soft signal; full coverage exists in `06_Clause_Mapping_Matrix.md`. **Out of Fase de Especificação 1 scope.**)
...
### 5.2 Issues NOT FIXED (out of Fase de Especificação 1 scope, deferred to Fase de Especificação 2/3)

| Issue ID | Description | Owner | Sprint |
|----------|-------------|-------|--------|
| NF-1 | Doc 07 coverage matrix has 41 entries (expected 38) | Corpus | Fase de Especificação 2 |
| NF-2 | 4 sole-authority gaps missing from Doc 07 gaps table | Doc 07 enrichment | Fase de Especificação 2/3 |
| NF-3 | D-02.3 sole authority GDPR vs CRA in `00_Taxonomy_Reference.md` | Ontology content (P7 human decision) | Fase de Especificação 3 |
| NF-4 | Only 3 security domains mentioned in `05_Regulatory_Applicability.md` | Doc 05 enrichment | Fase de Especificação 2 |
| NF-5 | 5 "Template not found" warnings (templates not shipped) | Template provisioning | Fase de Especificação 3 |
| **Total remaining** | | | **10 warnings** |

### 5.3 Fase de Especificação 1 task-level fix status
...
| 2.c | I-03 empty cells in tables | ⏭️ SKIPPED | All `—` cells in copied Phase 1 docs are template markers, not data gaps; legacy lint found no empty-cell warnings (LINT_REPORT_BEFORE §3 has zero such warnings). N/A for Fase de Especificação 1. |
...
| 2.f | I-08 UC count 35→62 marker | ⏭️ SKIPPED | No `35 UCs` reference in any copied Phase 1 doc; UC count lives in Phase 3 docs (`15_Requirements_Allocation.md`, `17_Functional_Tree.md`) which are out of scope for Fase de Especificação 1. N/A. |
| 2.g | I-09 `adapted_objectives.yaml` stubs | ⏭️ SKIPPED | The `review/adapted_objectives.yaml` is in `02_CASES/Case_01_TinyTask_SaaS/review/`, NOT in `01_PHASE1_CONTEXT/`. Fase de Especificação 1 task explicitly defers this to "Fase de Especificação 2+ territory". Not in scope. |
| 2.h | I-10 status DRAFT→RECONCILED | ✅ DONE | 10/11 docs updated (07b remains ACTIVE by design — it was signed off as ACTIVE in legacy, Fase de Especificação 1 reconciliation is content-neutral) |
...
| 4 | Update NEW docs (05b, Citation_Index) | ✅ DONE | Both remain PLACEHOLDER with `Skeleton ✓ Reconciled ✓` (Fase de Especificação 1 milestone) |
| 5 | Update skeleton README and placeholders | ✅ DONE | `README.md` §8 updated to reflect Fase de Especificação 0 complete / Fase de Especificação 1 IN_PROGRESS / per-doc status table |
| 6 | Run lints on Rich folder | ✅ DONE | This report |
| 7 | Write Fase de Especificação 1 completion report | ✅ DONE | See `validation/SPRINT1_REPORT.md` |
...
## 8. Notes for Fase de Especificação 2

- **8 of 10 remaining warnings are corpus-dependent** (Doc 07 coverage matrix over-count, sole-authority gap rows missing, D-02.3 ontology content). Fase de Especificação 2 should:
...
- **2 of 10 remaining warnings are tooling concerns** (Template not found, file rename side-effects). Fase de Especificação 3 should:
...
- **Phase 2/3 implications of the I-01 GDPR-C08 resolution.** The canonical mapping `GDPR-C08 = Art. 9 → D-05.3` (per `phase1_ontology.yaml v1.1`) means that:
  ...
  - The xlsx file `01_PHASE1_CONTEXT/06_Clause_Mapping_Matrix.xlsx → GDPR_MAPPING` sheet must be regenerated in Fase de Especificação 2 to reflect the new row layout (28 clauses still, but with `GDPR-C11 = Art. 24` instead of `GDPR-C08 = Art. 24`). The Fase de Especificação 1 markdown shim documents the canonical mapping; the xlsx is still a Fase de Especificação 2 deliverable.
