...
**Version:** 8.2 (MASTER DASHBOARD richer tables R7 — 1-2 colunas novas por tabela nas 9 DataTables (Type/Anchor/Threats/Activities/Type+Description/Threat+Type/Type/Likelihood+Impact) com tooltips `<th title="...">` em todas as células e KPI labels; modal drill-down ganha mini-TOC navegável (`.am-toc` com anchor links + scrollIntoView smooth) e filter input (`.am-filter`) que esconde secções + `<dt>/<dd>` flat fields que não casam, mostrando "No matches" quando nada bate; cross-dashboard links (`#folio{04..12}-crosslink`) com paths dinâmicos case-aware para `Case_0X_P{2,3}_Dashboard.html` e `UNIFIED/AEGIS_Dashboard.html`; CSS `.am-toc / .am-filter / .am-folio-crosslink / .am-section[hidden] / .am-no-matches`; `fillFolioCrosslinks(c)` em `renderP1/P2/P3`; test_master_drilldown.py R7: 9 column checks (header presence + non-placeholder data via DataTables API) + 1 modal TOC+filter (≥5 links, click scroll, "basic" filter hides non-matching) + 1 crosslink suite (9 anchors); regressão 9/9 R5 + 3/3 R6 mantida; screenshot `tests/screenshots/master_drilldown_toc.png`; ancillary: fixed Folio 06 thead vs ensureDT column mismatch (was using "Obligation ID/Title/Regulation/Maps to/Confidence" placeholder but data uses `Control/Framework/Anchor/ID`); unified `_caseShort(c)` resolver)
**Version:** 8.2 (MASTER DASHBOARD P1 complete R8 — 4 sub-tabs em #panel-p1 (Overview / Controls & Mappings / Ambiguity & Risk / Context & Compliance); 8 folios novos (F07 Controls & Frameworks 276 NIST CSF controls, F08 Clause Mapping 133 rows, F09 DORA ICT 23 articles C3-only, F10 Proportionality, F11 Posture Gaps, F12 Stakeholders+RACI, F13 Architecture, F14 Adjusted Goals 80 cards, F15 Citations); 12 novos parsers em build_master_dashboard_data.py (Doc07/08/10/11/12/13/14/Citation_Index); JSON 6.0→6.3 MB; smoke 17/17 + master + drilldown mantidos; 9 screenshots p1_{overview,controls,ambiguity} × 3 casos; MASTER DASHBOARD full-card drill-down R6 — quando carrego na linha do Master Dashboard, o modal mostra o body completo do card de origem (RUP §1..10 para UCs, 1..18 numbered fields para Rules, bold-label fields para FR/NFR, RISK-NN 17 fields + THR-... 12 fields, Card+Resolution para Ambiguity, mermaid block para PROC/CAP), não só as colunas achatadas da tabela; 6 novos parsers em `scripts/build_master_dashboard_data.py` — `parse_uc_cards` (RUP + compact §3), `parse_rule_cards` (1..18 numbered, C1/C2 only), `parse_req_cards` (bold-label C1 + table-rows C3), `parse_threat_cards` (RISK+THR 17/12 fields, C1 only), `parse_ambiguity_cards` (Card+Resolution pairing by ordinal), `parse_lane_diagram` (mermaid extraction) — wired `detail` em p1/p2/p3 rows; shell `renderMarkdownToHTML` (DOM-tree, sanitised: code fences / tables / numbered+bullet lists / `**Label:**` lines) + `showDetailModal` extended + CSS `.am-section / .am-kv / .am-table / .am-card-detail` + `activatePhase` fix (force re-render on case+phase combo change); JSON 4.7 MB→5.98 MB; smoke 17/17 + master 15/15 + drilldown 9/9 R5 + 3/3 R6 + 3 screenshots `master_drilldown_{uc,rule,ambiguity}.png`; ancillary bugfixes: `c` var shadowing in `build()` (caused JSON circular reference via CAP dict aliasing case dict), FR/NFR heading regex corrected to consume `### FR-NN — Title [priority=..., fields=N]` in full)
**Version:** 8.0 (MASTER DASHBOARD drill-down R5 — modal overlay CSS-only, click em qualquer linha abre vista detalhada com todos os campos do objecto (9 tabelas: folio02-amb, folio05-rules, folio06-obj, folio09-uc, folio10-proc-cap, folio11-req, folio12-threat, folio-p2-ambiguity, folio-p3-threat-ambiguity); ensureDT novo contract {title,key,render?}+objects; test_master_drilldown.py 9/9 OK; UX: Copy JSON, ESC, backdrop; 17/17 smoke + 15/15 master mantidos; MASTER DASHBOARD widget fix R4 — 17 paths/joins errados corrigidos no shell: Folio 3 evidence, 5 rules (46 rows), 6 obligations (34/15/27/122), 7 tensions (4 listadas, POSTURE=PLANNED), 8 framework join (NIST CSF+ASVS+SAMM+...), 10 owners filter, 11 requirements FR/NFR, 12 gates/threats table, 13 cross-case KPIs; gen emite last_updated + gates_matrix; 17/17 smoke + 15/15 master + 9 screenshots; single-file `AEGIS_Master_Dashboard.html` (6 CSF axes), vis-network@9 graph 749 nós, ambiguity widgets replicados P1↔P2↔P3, P2/P3 radares, 17/17 smoke + test_master_dashboard.py 15/15; JSON 122 KB→2.6 MB; single-file `AEGIS_Master_Dashboard.html` with 3-case × 3-phase + crosscase selector and 13 folios; data layer `scripts/build_master_dashboard_data.py` → `data/aegis_master_data.json` (103 UCs / 98 PROC / 23 CAP / 187 rules / 186 FR / 158 NFR); realization_class explicit yaml (C1) + documented heuristic (C2/C3); smoke 17/17 dashboards + 10 Playwright vistas match expected counts; screenshots in tests/screenshots/master_*.png; RENUMBER — flat 1..N lane ids, zero legacy: C3 UC 33→01..93→33 + PROC-41..52→39..50; C1 U.C.x.y.z flattened → UC-01..40; C2 → UC-01..36; rubric v1.10 §5B rule 7; registry RENUMBER_REGISTRY_2026-09-05; MERMAID RENDER FIX — all diagrams render-validated: 26 UC diagrams → PlantUML+SVG hybrid, rubric v1.9, mermaid check 192/192; UC SEPARATION — lane-pure use-case catalogs in 3 cases (rubric v1.8 §5B rule 6): C3 Doc22 v3.0 33 UCs + PKG-DS + PROC-39/40→UC-66/92; C1 Doc20 v3.2 23+17 UCs; C2 Doc21 v1.5 36 UCs; annex A UC-ovals-only 11/6/8 + annex B 23/21/33; audit 100/100/100; 6 borderline → P7; LANE CARDS — PROC/CAP card schemas + 12-card pilot; LANE NAMING — UC reserved to TECHNOLOGY; 102 UCs re-laned PROC/CAP, ~1.855 refs; OWASP threat×flow + Volere FR pilot; ALT-ANCHOR — UNMAPPED_* retired in 3 cases; 5 frozen referentials + generator + 3 gates v0.4 PASS; REALIZATION-CLASS — Case_01 P2 realization_class tag wave documents-only; PORT-PARITY-2 — cross-case content parity: corr-013 C3 renumber, C3 P1 v1.6 layer 749n/2054l, P2 waves C2+C3, P3 rich v0 C2+C3, dashboards 16/16 smoke)
**Scope:** All Cases
...
| Item | Value |
|------|-------|
| Diagnosis | Real-render validation (Playwright + Mermaid v11 latest, 218 blocks): 56 FAIL — `useCaseDiagram` does not exist in any stable Mermaid release (mermaid-js/mermaid#4628; the v1.7 §5C.5 "≥ v11.6" premise was false), and 30 sequence diagrams broke on `;` message separators / the `OFF` participant keyword collision with `autonumber off` |
| Decision (human) | Use-case diagrams → **PlantUML + SVG hybrid**: native `@startuml` source block (kept for editability) + committed SVG in `annexes/svg/` (rendered via the PlantUML server at authoring time), embedded as a markdown image. Mermaid stays for sequence diagrams and lane-card flowcharts. Codified: rubric v1.9 §5C.5 |
...
## 2.1 2026-09-07 — MASTER DASHBOARD FIX (radar + grafo + ambiguity widgets)

| Item | Value |
|------|-------|
| Diagnosis | Real-render validation (`scripts/mermaid_check.py` analogue) + `python3 -c 'MASTER_DATA.cases.X.phases.P1.maturity'` showed 13 widgets broken (radar, graph placeholder, ambiguity, etc.) — the JSON only had a few keys; everything else needed parsing from the corpus. |
| Decision (human) | Implement real graph + replicate ambiguity widgets P1↔P2↔P3 (no Cross-case fallback). |
| Gerador | `scripts/build_master_dashboard_data.py` extended with `parse_ambiguity` (Doc09 + phase1_graph.json:ambiguity.top_cards), `parse_maturity` (NIST CSF 2.0 6-function radar with `min(4, round(sqrt(count)*2))` deterministic scoring), `parse_graph_full` (P1 nodes/links 456/521/749; P2/P3 from control_set + lane cards), `parse_p2` extensions (rules_table/obligations/objectives/tensions/posture/frameworks/qnrcs), `parse_p3` extensions (fr_table/nfr_table/allocations/threat_table/gates_table/threat_flow/maturity_distribution). |
| Shell | `UNIFIED/AEGIS_Master_Dashboard.html`: added `<script src='vis-network@9'>` CDN; replaced Folio 4 placeholder with real force-directed graph (Full/Rules/Products sub-tabs); P1 Folio 3 radar reads `p1.maturity` and renders non-degenerate polygon; new P2/P3 radars (TECH/PROC/CAP and UC/PROC/CAP/FR axes); new P2 Realization Ambiguity widget + new P3 Threat Ambiguity widget; Folio 1 GRAPH NODES tile fix (counts vs arrays). |
| Verification | `python3 scripts/build_master_dashboard_data.py` → 2.7 MB JSON, 0 warnings; `python3 00_METHODOLOGY/00_VISUALISATIONS/tests/test_master_dashboard.py` → **15/15 OK** (3 cases × 5 counts: graph.nodes/links, ambiguity.open/rows, maturity.labels + vis-network=true + 5 canvases); `test_dashboards.py` → 17/17 PASS; 3 screenshots in `tests/screenshots/master_{case}_p1_radar_graph.png` visually confirm radar polygon + force-directed graph. |
| Commits | pending |
...
| Item | Value |
|------|-------|
| Schemas | `REALIZATION_CLASS_RUBRIC.md` v1.4 §5C — Process card (SSDF-style: Trigger/Activities/Roles/SLA/Realises/Anchors/Evidence) + Capability card (C2M2/ArchiMate-style: Owner/Span/Maturity Scale A/Realises/Anchors/Evidence). Chain: RULE → CAP → PROC → UC |
...
| Item | Value |
|------|-------|
| Pack | `00_METHODOLOGY/validation/P7_BRIEFING_PACK_2026-09-05.md` — 4 items (OBL-D-06.2-001 orphan / verify_rich FAILs / D-07.2 coverage C1 / phantom refs), each with 3 options and Orchestrator recommendation |
| Status | No implementation campaigns executed — material for human decision. Decisions pending; once ratified, follow-on campaigns can be planned. |
| Ledger | `02_CASES/PENDING_CAMPAIGNS_LEDGER.md` §5 keeps all 4 items as P7-pending with the pack's options surfaced |
