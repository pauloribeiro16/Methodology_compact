...
|----|--------|----------|-----------|--------|------------|------------|
| PROC-39: Configure Encryption | Attacker modifies encryption configuration | T (Tampering) | Medium | Critical | HIGH | Implement configuration validation and integrity checks |
| PROC-40: TLS Enforcement | Attacker downgrades TLS to weaker version | T (Tampering) | Medium | High | HIGH | Enforce TLS 1.3 with HSTS preload |
...
| UC | Threat | Category | Likelihood | Impact | Risk Score | Mitigation |
|----|--------|----------|-----------|--------|------------|------------|
| PROC-39: Configure Encryption | Linking encrypted data across systems | L (Linkability) | Low | Medium | LOW | Key segregation by domain |
| PROC-41: Field-Level Encryption | AI training data re-identification | I (Identifiability) | Medium | High | HIGH | Differential privacy; data minimization |
...
| PROC-43: Enforce MFA | Attacker bypasses MFA via real-time phishing | S (Spoofing) | High | High | CRITICAL | FIDO2 phishing-resistant auth; hardware keys for privileged |
...
| PROC-44: AI Model Access | Attacker exploits AI parameter change | E (Elevation) | Medium | Critical | HIGH | Dual approval for AI parameters; audit trail |
| PROC-45: FIDO2 Implementation | Attacker clones FIDO2 key | S (Spoofing) | Low | High | MEDIUM | Hardware key tamper resistance; key binding to device |

**LINDDUN Analysis:**

| UC | Threat | Category | Likelihood | Impact | Risk Score | Mitigation |
|----|--------|----------|-----------|--------|------------|------------|
| PROC-43: Enforce MFA | User unaware of MFA bypass attempt | U (Unawareness) | Medium | High | HIGH | Real-time notification on MFA anomalies |
...
| PROC-44: AI Model Access | AI access patterns reveal user behavior | D (Detectability) | Medium | Medium | MEDIUM | Aggregate AI access metrics; minimize per-user logging |
...
| UC-01: Data Erasure | Erasure request falsified by attacker | R (Repudiation) | Low | High | MEDIUM | Multi-factor request verification; audit trail |
| UC-02: Data Export | Export reveals AI decision logic | I (Information Disclosure) | Medium | High | HIGH | Differential privacy in exports; aggregation |
...
| UC-01: Data Erasure | Erasure conflicts with DORA log retention (T-002) | N (Non-compliance) | Medium | Critical | HIGH | Cryptographic sharding (resolved T-002) |
| UC-02: Data Export | Data subject learns about AI profiling | U (Unawareness) | Medium | Medium | MEDIUM | Transparency in AI decision explanations |
...
| PROC-46: CI/CD Security | Attacker injects malicious code in pipeline | T (Tampering) | Medium | Critical | HIGH | Pipeline integrity; signed commits; isolated build environments |
...
| PROC-47: AI Training Pipeline | Poisoned training data in CI/CD | T (Tampering) | Medium | Critical | HIGH | Training data validation; model signing; provenance |
| PROC-48: IaC Scanning | Attacker exploits misconfiguration before scan | T (Tampering) | Medium | High | HIGH | Pre-scan configuration validation; real-time scanning |
...
| PROC-47: AI Training Pipeline | Training data provenance reveals business logic | D (Detectability) | Medium | Medium | MEDIUM | Aggregate provenance; confidential training data |
...
| UC | Threat | Category | Likelihood | Impact | Risk Score | Mitigation |
|----|--------|----------|-----------|--------|------------|------------|
| PROC-49: AI Threat Detection | Attacker crafts attacks evading AI detection | I (Information Disclosure) | Medium | High | HIGH | Multi-layer detection; human analysis; threat intelligence |
...
| PROC-50: AI Model Drift | Drift detection evasion by sophisticated attackers | I (Information Disclosure) | Low | High | MEDIUM | Multiple drift detection methods; threshold tuning |
...
| UC | Threat | Category | Likelihood | Impact | Risk Score | Mitigation |
|----|--------|----------|-----------|--------|------------|------------|
| PROC-49: AI Threat Detection | AI monitoring reveals business operations | D (Detectability) | Medium | Medium | MEDIUM | Aggregate monitoring data; minimize operational details |
...
|-----------|---------|---------------|----------|
| PROC-43.1 | Enforce FIDO2 Hardware Keys for Privileged Access | R-01 | CRITICAL |
...
| PROC-46.1 | Implement Pipeline Integrity with Signed Commits and Isolated Builds | R-06 | HIGH |
...
|-------|------------|---------------|
| PROC-43 | Add step-up authentication for high-risk transactions | R-01 |
...
| PROC-46 | Add pipeline integrity controls with signed commits | R-06 |
...
1. **PROC-43.1**: Enforce FIDO2 Hardware Keys for Privileged Access
...
6. **PROC-46.1**: Implement Pipeline Integrity with Signed Commits
...
- PROC-43 «refine» PROC-43.1 (add FIDO2 hardware key detail)
