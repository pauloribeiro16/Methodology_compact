...
## 1K. DELIVERED — MASTER DASHBOARD P1 complete R8 (2026-09-09)

Human: «eu não vejo controlos da NIST nenhum na fase 1, eu visualizar todos os dados de todos os documentos aqui, verifica o que falta e organiza em como colocar, podes dentro de uma fase colocar mais toggles como fizeste para as fases». Diagnóstico: ~14 P1 artefactos não estavam no dashboard. Fix: 4 sub-tabs em #panel-p1 (Overview/Controls & Mappings/Ambiguity & Risk/Context & Compliance) + 8 folios novos (F07..F15) + F02 melhorado (coluna Detail preview); 12 novos parsers no gerador extraem de Doc07/08/10/11/12/13/14/Citation_Index. Smoke 17/17 + test_master + drilldown mantidos.

## 1J. DELIVERED — MASTER DASHBOARD richer tables R7 (2026-09-09)

Human: «quero que as tabelas do Master Dashboard e o modal mostrem mais informação, e adicionar links para os dashboards dedicados». Delivered: R7.1 — 1-2 colunas novas por tabela nas 9 DataTables (`folio02-amb` +Type, `folio05-rules` +Anchor, `folio06-obj` +Type, `folio09-uc` +Threats, `folio10-proc-cap` +Activities, `folio11-req` +Type+Description, `folio12-threat` +Threat+Type, `folio-p2-ambiguity` +Type, `folio-p3-threat-ambiguity` +Likelihood+Impact) — render functions conditional on source fields, fallback `—`. R7.2 — `<th title="...">` em todos os headers (ID/Title/Realization/NI/Source/Severity/Status/...). R7.3 — modal ganha mini-TOC (`.am-toc` com anchor links + `scrollIntoView({behavior:"smooth", block:"start"})`) e filter input (`.am-filter`) que esconde secções e pares `<dt>/<dd>` flat que não casam a query, com notice `.am-no-matches` quando nada bate — só visível quando `detail.sections.length > 3`. CSS `.am-toc / .am-filter / .am-folio-crosslink / .am-section[hidden] / .am-no-matches`. R7.4 — cross-dashboard links estáticos `#folio{04..12}-crosslink` preenchidos dinamicamente por `fillFolioCrosslinks(c)` (case-aware via `_caseShort`): Folio 4 → `UNIFIED/AEGIS_Dashboard.html`; 5/6/7 → `Case_0X_P2_Dashboard.html`; 8 → UNIFIED + GDPR + CRA + QNRCS; 9/10/11/12 → `Case_0X_P3_Dashboard.html`. R7.5 — test_master_drilldown.py: 9 R7.1 column checks (header presence via DOM + non-placeholder value via DataTables API) + 1 R7.3 modal TOC+filter (`Case_03 P3 folio09`: TOC ≥ 5 links, click scroll, typing "basic" hides non-matching sections) + 1 R7.4 crosslink suite (9 anchors `target="_self"`). Regression 9/9 R5 + 3/3 R6 mantida. Ancillary: corrigido Folio 06 thead vs ensureDT column mismatch (placeholder estava com colunas erradas); `_caseShort(c)` unificado para `caseKey`/`id`/`name`.

## 1I. DELIVERED — MASTER DASHBOARD full-card drill-down R6 (2026-09-07)

Human: «quando carrego na linha do Master Dashboard, o modal deve mostrar MUITO mais informação — o body completo do card de origem (RUP para UCs, campos numerados para Rules, Resolution para Ambiguity), não só as colunas achatadas da tabela». Delivered: 6 novos parsers no `build_master_dashboard_data.py` (`parse_uc_cards` RUP §1..10 + compact §3, `parse_rule_cards` 1..18 numbered fields, `parse_req_cards` bold-label fields C1 + table-rows C3, `parse_threat_cards` RISK-NN 17 fields + THR-... 12 fields, `parse_ambiguity_cards` Card+Resolution pairing by ordinal, `parse_lane_diagram` mermaid extraction) — wired `detail` em `p3.use_cases / p2.rules_table / p3.fr_table+nfr_table / p3.threat_table / p1.ambiguity_rows / p3.proc_cards / p3.cap_cards`. Shell: `renderMarkdownToHTML` (DOM-tree helper, sanitised, code fences / tables / numbered+bullet lists / `**Label:**` lines) + extensão de `showDetailModal` + CSS `.am-section / .am-kv / .am-table / .am-card-detail` + fix `activatePhase` para forçar re-render quando muda case+phase combo. JSON 4.7 MB → 5.98 MB. Smoke 17/17, master 15/15, drilldown 9/9 R5 + 3/3 R6 (UC sections + numbered Basic Flow, Rule 26 fields, Ambiguity Card+Resolution); 3 screenshots `master_drilldown_{uc,rule,ambiguity}.png`. Ancillary bugfixes: `c` variable shadowing in `build()` (was aliasing case dict into CAP row, caused `JSON circular reference`), FR/NFR regex corrected to consume `### FR-NN — Title [priority=..., fields=N]` in full.
