...
# Sprint 2 Report — Multi-Paragraph Tensions Expansion (Case_02 Phase 2 Rich Mode)
...
|---------|-------------|-------------|-------------|-------------------|
| **T-001** (D-04.3) | GDPR Art. 33(1) controller 72h obligation + corpus `GDPR-CP17` text | CRA Art. 14(1) + Art. 14(2)(a) manufacturer 24h/72h/14d obligation + corpus `CRA-CL51`+`CRA-CL52`+`CRA-CL53` text | Why the two clocks collide when one event triggers both — 3 realistic Case_02 scenarios | ✅ GDPR Art. 33(1) "not later than 72 hours" + CRA Art. 14(1) "within 24 hours" + CRA Art. 14(2)(a)/(b)/(c) 24h/72h/14d |
...
| **T-L-001** (DORA↔GDPR) | DORA Art. 9(4)(a) immutable log + Art. 17 5-year retention + corpus `DORA-CL31`+`DORA-CL41`+ family text | GDPR Art. 17 right to erasure + corpus `GDPR-CL05` text | Why the conflict is structural in nature but INACTIVE for Case_02 (DORA Art. 2(1) scope) | ✅ DORA Art. 9(4)(a) "ICT-related activities … shall be logged and the logs shall be traceable" + GDPR Art. 17 "the data subject shall have the right to obtain from the controller the erasure of personal data" |
...
```
Branch: feature/aegis-p2-case02-rich
Base: main
Sprint: 2
...
```bash
# File size check
wc -l 02_CASES/Case_02_SecureBorder_Solutions/02_PHASE2_RULES_RICH/09_Strategic_Tensions_Report.md
# Result: 686 lines (≥ 600 target)

# Tension heading check
grep -cE "^### T-" 02_CASES/Case_02_SecureBorder_Solutions/02_PHASE2_RULES_RICH/09_Strategic_Tensions_Report.md
# Result: 4 (T-001, T-M-001, T-M-002, T-L-001)

# Root Cause Analysis section check
grep -c "Root Cause" 02_CASES/Case_02_SecureBorder_Solutions/02_PHASE2_RULES_RICH/09_Strategic_Tensions_Report.md
# Result: 5 (1 schema intro + 4 detailed sections)

# Field subheading check (8 fields × 4 tensions = 32)
grep -cE "^#### §5\.[1-4]\." 02_CASES/Case_02_SecureBorder_Solutions/02_PHASE2_RULES_RICH/09_Strategic_Tensions_Report.md
...
for tension in "T-001" "T-M-001" "T-M-002" "T-L-001"; do
  count=$(sed -n "/^### $tension /,/^### \|^## /p" 02_CASES/Case_02_SecureBorder_Solutions/02_PHASE2_RULES_RICH/09_Strategic_Tensions_Report.md | grep -cE "^\*\*Paragraph [0-9]")
  echo "$tension: $count paragraphs"
done
# Result: T-001: 3, T-M-001: 3, T-M-002: 3, T-L-001: 3 (total 12)

# Validation report line count
wc -l 02_CASES/Case_02_SecureBorder_Solutions/02_PHASE2_RULES_RICH/validation/SPRINT2_REPORT.md
...
**End of Sprint 2 Report — Multi-Paragraph Tensions Expansion (Case_02 Phase 2 Rich Mode)**
