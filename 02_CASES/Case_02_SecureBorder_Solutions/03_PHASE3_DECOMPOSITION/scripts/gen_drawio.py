#!/usr/bin/env python3
"""
gen_drawio.py — Phase 3 Rich Mode functional-tree diagram, Case_02 (SecureBorder
Solutions). PORT-PARITY-2 block F5 port of Case_01's script of the same name.

Reads the ```mermaid``` block in Doc27_Functional_Tree.md and emits a parseable
...
Usage:
  ...
  python3 scripts/gen_drawio.py --input Doc27_Functional_Tree.md --output 18_Functional_Tree.drawio
"""
...
def render_drawio(nodes: Dict[str, str], edges: List[Tuple[str, str]]) -> str:
    ...
    cells.append('  <diagram id="case02-phase3-functional-tree" name="Case_02 Functional Tree">')
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Generate .drawio from Mermaid in Doc27_Functional_Tree.md (Case_02)")
    parser.add_argument("--input", default=None, help="Path to functional-tree .md (default: ../Doc27_Functional_Tree.md)")
    parser.add_argument("--output", default=None, help="Output .drawio (default: ../18_Functional_Tree.drawio)")
    args = parser.parse_args()

    base = Path(__file__).resolve().parent.parent
    input_path = Path(args.input) if args.input else base / "Doc27_Functional_Tree.md"
